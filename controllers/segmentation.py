...
def segmentation(df, main, index):
    ...
    if not is_numeric_dtype(df[main]):
        raise ValueError(f"Target column '{main}' must be numeric")

    features = [x for x in df.columns if x not in [index, main] and is_numeric_dtype(df[x])]

    histogram = {}
    for column in features:
        if is_numeric_dtype(df[column]):
            hist = np.histogram(df[column], bins=10)
            hist_x = [nicefy(hist[1][range_]) + '-' + nicefy(hist[1][range_ + 1]) for range_ in range(len(hist[1]) - 1)]
            y = hist[0]
        else:
            hist = dict(Counter(df[column].values.tolist()))
            hist_x = list(hist.keys())
            y = list(hist.values())
        histogram[column] = {'x': hist_x, 'y': y}

    # Ensure only numerical columns are used for correlation
    numeric_df = df[features + [main]]
    corr_heatmap = {
        'values': numeric_df.corr().values,
        'columns': numeric_df.corr().columns.tolist()
    }

    estimator = DecisionTreeRegressor().fit(df[features].values, df[main].values)
    feature_importance = {
    ...
    for feature in features:
        ...
        df[f'score of {feature}'] = df[f'score of {feature}'].values.astype('str')
        ...
        df[f'score of {feature}'] = df[f'score of {feature}'].values.astype('int')
